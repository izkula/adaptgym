...
class Balance(base.Task):
  ...
  def initialize_episode(self, physics, step):
    ...
    if step >= self._unconstrain_at_step:
      print(f'step {step}')
      physics.model.jnt_limited[1] = 0
      physics.model.jnt_range[1, 0] = 0
      physics.model.jnt_range[1, 1] = 0
      # physics.model.jnt_range[1, 0] = -100 #3.05433 #0
      # physics.model.jnt_range[1, 1] = 100  #3.22886 #5
      physics.forward()
