...
class NullGoalMaze(composer.Task):
  ...
  def __init__(self,
    ...
    self._walker.observables.egocentric_camera.enabled = False
    ...
    self.root_entity.mjcf_model.size.nconmax = nconmax
    self.root_entity.mjcf_model.size.njmax = njmax
  ...
  def _respawn(self, physics, random_state):
    ...
    if self._randomize_spawn_position:
      ...
      if isinstance(self._maze_arena.spawn_positions, dict):
          key = list(self._maze_arena.spawn_positions.keys())[0]
          self._spawn_position = self._maze_arena.spawn_positions[key][0]
      else:
