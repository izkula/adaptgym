...
class MazeWithTargets(composer.Arena):
  ...
  def _build(self, maze, xy_scale=2.0, z_height=2.0,
             ...
             aesthetic='default', name='maze', agent_spawn_tokens=(labmaze.defaults.SPAWN_TOKEN),
             wall_heights=None, wall_positions=None, border_scale=6,
             indiv_squares=True):
    ...
    self._z_height_scale = z_height
    self._border_scale = border_scale
    self._agent_spawn_tokens = agent_spawn_tokens
    self._indiv_squares = indiv_squares

    self._wall_heights = wall_heights
    self._wall_positions = wall_positions
    if wall_positions is not None:
      self._wall_positions = text_grid.TextGrid(self._wall_positions)
    ...
    self.ground_x = ((self._maze.width - 1) + 1) * (xy_scale / 2)
    self.ground_y = ((self._maze.height - 1) + 1) * (xy_scale / 2)
    self._mjcf_root.worldbody.add(
        'geom', name='ground', type='plane',
        pos=[0, 0, 0], size=[self.ground_x, self.ground_y, 1], rgba=[0, 0, 0, 0])
  ...
  def regenerate(self, random_state=np.random.RandomState(), random_seed=None):
    ...
    ##### Specify maze wall heights  ### THIS IS JANKY RIGHT NOW. ### THINK ABOUT THIS MORE!
    if self._wall_positions is None:
      walls = np.array(self._maze.entity_layer == _DEFAULT_WALL_CHAR, dtype='int')

    if self._wall_heights is None:
        z_height = self._z_height_scale*np.random.random(walls.shape)*walls
    else:
        if isinstance(self._wall_heights, (int, float)):
            z_height = self._wall_heights * walls
        elif isinstance(self._wall_heights, str):
          heights = text_grid.TextGrid(self._wall_heights)
          wall_heights = np.zeros(heights.shape)
          for x, y in itertools.product(range(heights.shape[0]), range(heights.shape[1])):
            h = heights[x, y]
            if h.isdigit():
              wall_heights[x, y] = int(h)
          z_height = self._z_height_scale*wall_heights/9
            # raise('String-specified wall heights Not yet implemented!')
    border_height = self._z_height_scale*self._border_scale
    z_height[0, :] = border_height
    z_height[-1, :] = border_height
    z_height[:, 0] = border_height
    z_height[:, -1] = border_height
    self._z_height = z_height
    ...
    for wall_char in self._wall_textures:
      self._make_wall_geoms(wall_char,  self._wall_positions, indiv_squares=self._indiv_squares)
    self._make_floor_variations()

  def _make_wall_geoms(self, wall_char, wall_positions=None, indiv_squares=True):
    if wall_positions is None:
      wall_positions = self._maze.entity_layer
    walls = covering.make_walls(
        wall_positions, wall_char=wall_char, make_odd_sized_walls=True,
        indiv_squares=indiv_squares)
    for i, wall in enumerate(walls):
      wall_mid = covering.GridCoordinates(
          (wall.start.y + wall.end.y - 1) / 2,
          (wall.start.x + wall.end.x - 1) / 2)
      if isinstance(self._z_height, float) or isinstance(self._z_height, int):
          z_height = self._z_height
      else:
          z_height = self._z_height[int(wall.start.y), int(wall.start.x)]
  ...
  def _find_spawn_and_target_positions(self):
    ...
    grid_positions = self.find_token_grid_positions(
        [labmaze.defaults.OBJECT_TOKEN] + list(self._agent_spawn_tokens))
    ...
    self._spawn_grid_positions = {key: value
                                  for key, value in grid_positions.items()
                                  if key in self._agent_spawn_tokens}
    self._spawn_positions = {key: self.grid_to_world_positions(value)
                             for key, value in self._spawn_grid_positions.items()}
