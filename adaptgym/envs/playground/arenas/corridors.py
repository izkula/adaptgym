...
class EmptyCorridor(Corridor):
  ...
  def _build(self,
    ...
    self._ground_plane = self._mjcf_root.worldbody.add(
        'geom', type='plane', rgba=[0.9, 0.5, 0.5, 1], size=[1, 1, 1])
    ...
    ### Moving this code here from GapsCorridor
    ...
    if self._aesthetic != 'default':
      ...
      for texture in self._mjcf_root.asset.find_all('texture'):
        if texture.type == 'skybox':
            texture.remove()
      if aesthetic == 'checker':
        self._ground_texture = self._mjcf_root.asset.add(
              'texture', type='2d', builtin='checker',
              rgb1=[0.9, 0.5, 0.5], rgb2=[0.3, 0.3, 0.3],  width=300, height=300)
        self._skybox = self._mjcf_root.asset.add(
             'texture', type='skybox', builtin='gradient',
             rgb1=[0.1, 0.3, 0.5], rgb2=[1.0, 1.0, 1.0], width=100, height=600)
      else:
  ...
  def regenerate(self, random_state):
    ...
    ### ADDING THIS TO ENABLE TEXTURE IN EMPTY CORRIDOR ###
    self._ground_body.geom.clear()
    ### Move existing ground plane down and make it invisible.
    ...
    if self._aesthetic != 'default':
        self._ground_body.add(
          ...
          type='plane',
          name='start_floor',
          pos=[self._current_corridor_length / 2, 0, 0],
          size=self._ground_plane.size,
