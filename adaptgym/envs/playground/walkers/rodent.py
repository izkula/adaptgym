...
class Rat(legacy_base.Walker):
  ...
  def aliveness(self, physics):
    return (physics.bind(self.root_body).xmat[-1] - 1.) / 2.   # Upside down is -1, right side up is 0
    # return 0.0  # The default

  def accel(self, physics):
    sensor = self._mjcf_root.find('sensor', 'accelerometer')
    a = np.array(physics.bind(sensor).sensordata)
    return a

  def vel(self, physics):
    sensor = self._mjcf_root.find('sensor', 'velocimeter')
