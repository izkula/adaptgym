...
class Wrapper(dm_env.Environment):
  ...
  def __init__(self, env, pixels_only=True, render_kwargs=None,
               observation_key='pixels', delete_pixel_observation=False):
    ...
    Args:
      ...
      delete_pixel_observation: bool. If True, still render the pixel observation
        (which may be necessary for properly setting up the mujoco environment to render the background),
        but delete it from the observation dict.
    ...
    self._delete_pixel_observation = delete_pixel_observation

  def reset(self):
    time_step = self._env.reset()
    # return self._add_pixel_observation(time_step)
    time_step = self._add_pixel_observation(time_step)
    if self._delete_pixel_observation:
      del time_step.observation['pixels']
    return time_step
